...
收到“回测 YYYY-MM-DD 所有比赛”或日期范围命令时，每条新命令都必须生成新的随机 request_id，并按日期重新采集、屏蔽赛果、重新运行完整模型、生成新的 Prediction Commit，再读取真实赛果完成评价和改进方案。不得直接调用旧 Prediction Commit、旧采集快照或旧报告；旧任务无论成功、失败或未完成都不能阻止新命令。日期范围逐日完整执行，最多 3 天。
